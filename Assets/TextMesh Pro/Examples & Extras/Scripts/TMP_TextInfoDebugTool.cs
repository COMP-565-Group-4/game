...
// TODO: Rework this script to make it into an editor utility.
    ...
    void OnDrawGizmos()
    {
        ...
        ObjectStats = "Characters: " + m_TextInfo.characterCount
            + "   Words: " + m_TextInfo.wordCount + "   Spaces: " + m_TextInfo.spaceCount
            + "   Sprites: " + m_TextInfo.spriteCount + "   Links: " + m_TextInfo.linkCount
    ...
    void DrawCharactersBounds()
    {
        ...
        for (int i = 0; i < characterCount; i++) {
            ...
            // Draw Ascent line
            Vector3 ascentlineStart =
                m_Transform.TransformPoint(new Vector3(origin, ascentline, 0));
            ...
            Vector3 capHeightEnd = new Vector3(
                topRight.x, m_Transform.TransformPoint(new Vector3(0, capline, 0)).y, 0
            );
            ...
            Vector3 meanlineStart = new Vector3(
                topLeft.x, m_Transform.TransformPoint(new Vector3(0, meanline, 0)).y, 0
            );
            Vector3 meanlineEnd = new Vector3(
                topRight.x, m_Transform.TransformPoint(new Vector3(0, meanline, 0)).y, 0
            );
            ...
            // Draw Descent line
            Vector3 descentlineStart =
                m_Transform.TransformPoint(new Vector3(origin, descentline, 0));
            Vector3 descentlineEnd =
                m_Transform.TransformPoint(new Vector3(advance, descentline, 0));
            ...
            if (m_HandleSize < 0.5f) {
                ...
                if (characterInfo.isVisible) {
                    ...
                    // Handles.Label(labelPosition, "Cap Line (" + caplineMetrics.ToString("f3") +
                    // ")" , style);
                    ...
                    // Handles.Label(labelPosition, "Mean Line (" + ascentlineMetrics.ToString("f3")
                    // + ")" , style);
            ...
            for (int j = 0; j < wInfo.characterCount; j++) {
                ...
                if (isBeginRegion == false && isCharacterVisible) {
                    ...
                    if (wInfo.characterCount == 1) {
                        ...
                        topLeft =
                            m_Transform.TransformPoint(new Vector3(topLeft.x, maxAscender, 0));
                        bottomLeft =
    ...
    void DrawLineBounds()
    {
        ...
        for (int i = 0; i < lineCount; i++) {
            ...
            TMP_CharacterInfo firstCharacterInfo =
                m_TextInfo.characterInfo[lineInfo.firstCharacterIndex];
            TMP_CharacterInfo lastCharacterInfo =
                m_TextInfo.characterInfo[lineInfo.lastCharacterIndex];

            bool isLineVisible =
                (lineInfo.characterCount == 1
                 && (firstCharacterInfo.character == 10 || firstCharacterInfo.character == 11
            ...
            // Draw Base line
            Vector3 baseLineStart =
                m_Transform.TransformPoint(new Vector3(lineBottomLeft, baseline, 0));
            Vector3 baseLineEnd =
                m_Transform.TransformPoint(new Vector3(lineTopRight, baseline, 0));
    ...
    void DrawDottedRectangle(Vector3 bottomLeft, Vector3 topRight, Color color, float size = 5.0f)
    {
        ...
        Handles.DrawDottedLine(
            bottomLeft, new Vector3(bottomLeft.x, topRight.y, bottomLeft.z), size
        );
        ...
        Handles.DrawDottedLine(
            new Vector3(topRight.x, bottomLeft.y, bottomLeft.z), bottomLeft, size
        );
    }
