...
public class SkewTextExample : MonoBehaviour
{
    ...
    IEnumerator WarpText()
    {
        ...
        while (true) {
            ...
            float boundsMinX =
                m_TextComponent.bounds.min.x; // textInfo.meshInfo[0].mesh.bounds.min.x;
            float boundsMaxX =
                m_TextComponent.bounds.max.x; // textInfo.meshInfo[0].mesh.bounds.max.x;
            ...
            for (int i = 0; i < characterCount; i++) {
                ...
                // float offsetY = VertexCurve.Evaluate((float)i / characterCount + loopCount /
                // 50f); // Random.Range(-0.25f, 0.25f);
                ...
                Vector3 topShear = new Vector3(
                    shear_value
                        * (textInfo.characterInfo[i].topRight.y - textInfo.characterInfo[i].baseLine
                        ),
                    0,
                    0
                );
                ...
                // Vector3 normal = new Vector3(-(y1 - y0), (x1 * (boundsMaxX - boundsMinX) +
                // boundsMinX) - offsetToMidBaseline.x, 0);
                ...
                matrix = Matrix4x4.TRS(
                    new Vector3(0, y0, 0), Quaternion.Euler(0, 0, angle), Vector3.one
                );
