...
public class TMP_TextSelector_A : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    void Awake()
    {
        ...
        // Force generation of the text object so we have valid data to work with. This is needed
        // since LateUpdate() will be called before the text object has a chance to generated when
        // entering play mode.
    ...
    void LateUpdate()
    {
        ...
        if (TMP_TextUtilities.IsIntersectingRectTransform(
                m_TextMeshPro.rectTransform, Input.mousePosition, Camera.main
            ))
        {
...
#region Example of Character Selection
            int charIndex = TMP_TextUtilities.FindIntersectingCharacter(
                m_TextMeshPro, Input.mousePosition, Camera.main, true
            );
...
#region Example of Word Selection
            ...
            int wordIndex = TMP_TextUtilities.FindIntersectingWord(
                m_TextMeshPro, Input.mousePosition, Camera.main
            );
            if (wordIndex != -1 && wordIndex != m_lastWordIndex) {
                ...
                // Debug.Log("Mouse Position: " + Input.mousePosition.ToString("f3") + "  Word
                // Position: " + wordPOS.ToString("f3"));
                ...
                for (int i = 0; i < wInfo.characterCount; i++) {
                    int vertexIndex =
                        m_TextMeshPro.textInfo.characterInfo[wInfo.firstCharacterIndex + i]
                            .vertexIndex;
