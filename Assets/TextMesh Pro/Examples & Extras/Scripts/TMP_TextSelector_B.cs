...
#pragma warning disable 0618 // Disabled warning due to SetVertices being deprecated until new
                             // release with SetMesh() is available.
...
#region Handle Character Selection
                ...
                && (Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift)))
            {
                ...
                int materialIndex =
                    m_TextMeshPro.textInfo.characterInfo[charIndex].materialReferenceIndex;
                ...
                // Need to translate all 4 vertices of the character to aligned with middle of
                // character / baseline. This is needed so the matrix TRS is applied at the origin
                // for each character.
                ...
                m_matrix =
                    Matrix4x4.TRS(Vector3.zero, Quaternion.identity, Vector3.one * zoomFactor);
                ...
                // Swap the current character's vertex attributes with those of the last element in
                // the vertex attribute arrays. We do this to make sure this character is rendered
                // last and over other characters.
...
#region Word Selection Handling
            ...
            int wordIndex = TMP_TextUtilities.FindIntersectingWord(
                m_TextMeshPro, Input.mousePosition, m_Camera
            );
                ...
                && (wordIndex == -1 || wordIndex != m_selectedWord))
            {
                ...
                for (int i = 0; i < wInfo.characterCount; i++) {
                    ...
                    // Get the index of the material / sub text object used by this character.
                    int meshIndex =
                        m_TextMeshPro.textInfo.characterInfo[characterIndex].materialReferenceIndex;

                    // Get the index of the first vertex of this character.
                    int vertexIndex =
                        m_TextMeshPro.textInfo.characterInfo[characterIndex].vertexIndex;
...
#region Example of Link Handling
            ...
            if (linkIndex != -1 && linkIndex != m_selectedLink) {
                ...
                // Debug.Log("Link ID: \"" + linkInfo.GetLinkID() + "\"   Link Text: \"" +
                // linkInfo.GetLinkText() + "\""); // Example of how to retrieve the Link ID and
                // Link Text.
...
/*
int charIndex = TMP_TextUtilities.FindIntersectingCharacter(m_TextMeshPro, Input.mousePosition,
m_Camera, true); if (charIndex != -1 && charIndex != m_lastIndex)
{
    //Debug.Log("Character [" + m_TextMeshPro.textInfo.characterInfo[index].character + "] was
selected at POS: " + eventData.position); m_lastIndex = charIndex;

    Color32 c = new Color32((byte)Random.Range(0, 255), (byte)Random.Range(0, 255),
(byte)Random.Range(0, 255), 255); int vertexIndex =
m_TextMeshPro.textInfo.characterInfo[charIndex].vertexIndex;
...
// Check if Mouse intersects any words and if so assign a random color to that word.
/*
int wordIndex = TMP_TextUtilities.FindIntersectingWord(m_TextMeshPro, Input.mousePosition,
m_Camera);

// Clear previous word selection.
if (m_TextPopup_RectTransform != null && m_selectedWord != -1 && (wordIndex == -1 || wordIndex !=
m_selectedWord))
{
    TMP_WordInfo wInfo = m_TextMeshPro.textInfo.wordInfo[m_selectedWord];
    ...
    for (int i = 0; i < wInfo.characterCount; i++)
    {
        int vertexIndex = m_TextMeshPro.textInfo.characterInfo[wInfo.firstCharacterIndex +
i].vertexIndex;
...
/*
// Check if Mouse intersects any words and if so assign a random color to that word.
int linkIndex = TMP_TextUtilities.FindIntersectingLink(m_TextMeshPro, Input.mousePosition,
m_Camera); if (linkIndex != -1)
{
    TMP_LinkInfo linkInfo = m_TextMeshPro.textInfo.linkInfo[linkIndex];
    ...
    Color32 c = new Color32((byte)Random.Range(0, 255), (byte)Random.Range(0, 255),
(byte)Random.Range(0, 255), 255); for (int i = 0; i < linkInfo.characterCount; i++)
    {
        TMP_CharacterInfo cInfo = m_TextMeshPro.textInfo.characterInfo[linkInfo.firstCharacterIndex
+ i];
