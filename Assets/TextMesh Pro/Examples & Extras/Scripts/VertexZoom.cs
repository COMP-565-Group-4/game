...
public class VertexZoom : MonoBehaviour
{
    ...
    {
        // We force an update of the text object since it would only be updated at the end of the
        // frame. Ie. before this code is executed on the first frame. Alternatively, we could yield
        // and wait until the end of the frame when the text object will be generated.
        ...
        while (true) {
            ...
            for (int i = 0; i < characterCount; i++) {
                ...
                // Get the cached vertices of the mesh used by this text element (character or
                // sprite).
                Vector3[] sourceVertices = cachedMeshInfoVertexData[materialIndex].vertices;

                // Determine the center point of each character at the baseline.
                // Vector2 charMidBasline = new Vector2((sourceVertices[vertexIndex + 0].x +
                // sourceVertices[vertexIndex + 2].x) / 2, charInfo.baseLine);
                // Determine the center point of each character.
                Vector2 charMidBasline =
                    (sourceVertices[vertexIndex + 0] + sourceVertices[vertexIndex + 2]) / 2;

                // Need to translate all 4 vertices of each quad to aligned with middle of character
                // / baseline. This is needed so the matrix TRS is applied at the origin for each
                // character.
                ...
                // Vector3 jitterOffset = new Vector3(Random.Range(-.25f, .25f), Random.Range(-.25f,
                // .25f), 0);
                ...
                // matrix = Matrix4x4.TRS(jitterOffset, Quaternion.Euler(0, 0, Random.Range(-5f,
                // 5f)), Vector3.one * randomScale);
                matrix = Matrix4x4.TRS(
                    new Vector3(0, 0, 0), Quaternion.identity, Vector3.one * randomScale
                );
